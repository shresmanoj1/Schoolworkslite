...
class _ParentFeeScreenState extends State<ParentFeeScreen> {
  ...
  transactionStart() async {
    ...
    // final res = await FeeService().duedatafinish(data);
    // for (int i = 0; i < res.data!.payments!.length; i++) {
    //   setState(() {
    //     total = total + double.parse(res.data?.payments?[i]['amount_paid']);
    //   });
    ...
    // for (int i = 0; i < res.data!.payments!.length; i++) {
    //   setState(() {
    //     tax = tax + double.parse(res.data?.payments?[i]['edu_tax']);
    //   });
    // }
    //
    // for (int i = 0; i < res.data!.payments!.length; i++) {
    //   setState(() {
    //     discount = discount +
    //         double.parse(res.data?.payments?[i]['referred_discount']);
    //   });
    // }
    ...
    //     final data = GetFeesForParentsRequest(
    //         studentId: user?.email.toString(),
    //         institution: user?.institution.toString());
    //
    //     start_response = ParentFeeService().financialDataStart(data);
    //     start_response = ParentFeeService().financialDataStart(data);
    //
    //     finish_response = ParentFeeService().financialDatafinish(data);
    //     final res = await ParentFeeService().financialDatafinish(data);
    //     for (int i = 0; i < res.data!.payments!.length; i++) {
    //       setState(() {
    //         total = total + double.parse(res.data?.payments?[i]['amount_paid']);
    //       });
  ...
  //   }
  //
  //   // print("The data is on way:::::"+jsonEncode(datas.data?.studentDetail));
  // }

  // financialDataFinish() async {
  //   final data = GetFeesForParentsRequest(
  //       studentId: user?.username.toString(),
  //       institution: user?.institution.toString());
  //
