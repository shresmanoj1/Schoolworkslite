...
class AssessmentSubmissionScreen extends StatefulWidget {
  final batch;
  final id;
  const AssessmentSubmissionScreen({Key? key, this.batch, this.id})
      : super(key: key);

  @override
  _AssessmentSubmissionScreenState createState() =>
      _AssessmentSubmissionScreenState();
}

class _AssessmentSubmissionScreenState
    extends State<AssessmentSubmissionScreen> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      _provider = Provider.of<LecturerCommonViewModel>(context,
          listen: false);
      _provider.fetchsubmissionstats(widget.id, widget.batch);
      _provider.fetchSubmissionCheck(widget.id, widget.batch);
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<LecturerCommonViewModel>(
        builder: (context, value, child) {
          return isLoading(value.submissionStatsApiResponse) || isLoading(value.submissionCheckApiResponse)
              ? const Center(
                                  ...
                                  Checkbox(
                                    ...
                                    onChanged: (value2) {
                                      setState(() {
                                        ...
                                        setState(() {
                                          if (checkStudent) {
                                            selectedStudents.addAll(value.submission.map((e) => e.username!).toList());
                                            checkStudent = true;
                                          } else {
                                                ...
                                                : IconButton(
                                                    onPressed: () {
                                                      Navigator.push(
                                                          context,
                                                          MaterialPageRoute(
                                                            builder: (context) => SubmissionFeedbackScreen(
                                                                data: value
                                                                    .submission[
                                                                        index]
                                                                    .submission
                                              ...
                                              child: Builder(
                                                builder: (context) {
                                                  try{
                                                    ...
                                                    selectedStudents = value.submissionCheck.students!;
                                                  ...
                                                  return Checkbox(
                                            value: selectedStudents.contains(value.submission[index].username),
                                            onChanged:
                                                    (bool? value3) {
                                                  setState(() {
                                                    if (value3!) {
