...
class PrinicpalCommonViewModel extends ChangeNotifier {
  ...
  Future<void> fetchdailystats() async {
    ...
    try {
      ...
      if (res.success == true) {
        _totalAttendance = res.totalAttendance!;
        _totalPresent = res.totalPresent!;
        _totalStudents = res.totalStudents!;
        _remainingAttendance = res.remainingAttendance!;
  ...
  // List<dynamic> _allSearchedStaffs = [];
  // List<dynamic> get allSearchedStaffs => _allSearchedStaffs;
  //
  // List<dynamic> _searchedStaffs = [];
  // List<dynamic> get searchedStaffs => _searchedStaffs;
  //
  // void searchStaffs(String searchText) {
  //   if (searchText == null || searchText.isEmpty || searchText == "") {
  //     _searchedStaffs.clear();
  //     _searchedStaffs.addAll(_allSearchedStaffs);
  //     // _allSearchedStaffs = List.from(_searchedStaffs);
  //   } else {
  //     _allSearchedStaffs = _searchedStaffs
  //         .where((staff) => (staff["firstname"] + " " + staff["lastname"])
  //             .toLowerCase()
  //             .contains(searchText.toLowerCase()))
  //         .toList();
  //     print("STAFF SEARCHED::${searchText}:::${_searchedStaffs}");
  //   }
  //   notifyListeners();
  // }
  ...
  Future<void> fetchstaffs() async {
    ...
    try {
      ...
      if (res.success == true) {
        ...
        // _searchedStaffs.clear();
        // _searchedStaffs.addAll(_staffs);
        // _allSearchedStaffs.addAll(_staffs);
  ...
  Future<void> fetchAbsentStudentDailyAttendance(String date) async {
    ...
    try {
      StudentDailyAttendanceResponse res = await AttendanceRepository()
          .getAbsentStudentDailyAttendanceReport(date);
      if (res.success == true) {
        _absentStudents = res;
        allAbsentStudents.clear();
        for (var i = 0; i < _absentStudents.dailyStdAttendance.length; i++) {
          for (var j = 0;
              j <
                  _absentStudents
                      .dailyStdAttendance[i]["absentStudents"].length;
              j++) {
            if (!allAbsentStudents.contains(_absentStudents
                .dailyStdAttendance[i]["absentStudents"][j]["username"])) {
              _allAbsentStudents.add({
                "full_name": _absentStudents.dailyStdAttendance[i]
                        ["absentStudents"][j]["firstname"] +
                    " " +
                    _absentStudents.dailyStdAttendance[i]["absentStudents"][j]
                        ["lastname"],
                "course": _absentStudents.dailyStdAttendance[i]
                    ["absentStudents"][j]["course"],
                "username": _absentStudents.dailyStdAttendance[i]
                    ["absentStudents"][j]["username"],
                "batch": _absentStudents.dailyStdAttendance[i]["batch"],
                "status": "Absent",
                "moduleData": {
                  "moduleTitle": _absentStudents.dailyStdAttendance[i]
                      ["moduleData"]["moduleTitle"],
                  "alias": _absentStudents.dailyStdAttendance[i]["moduleData"]
                      ["alias"]
                }
              });
            }
  ...
  Future<void> fetchAccessedModules(data) async {
    ...
    try {
      ...
      if (res.success == true) {
        _accessModules = res;
        print("MMMMMMM:::" + jsonEncode(_accessModules));
      ...
      _accessModules = new AccessedModulesResponse();
      print("VM CATCH ERR MODULE :: " + e.toString());
  ...
  Future<void> fetchMyPermissions() async {
    ...
    try {
      ...
      print("RESPONSE::::12345:::$res");
      print("RESPONSE::::12345:::${res["success"]}");
